	...
	timed!("best_plays", {
		...
		timed!("best_plays > rendering", {
			...
			for (i, origin) in item_origins.enumerate() {
				...
				let top_left_center =
					(drawer.layout.width(top_left_area) as i32 + jacket_margin) / 2;
		...
		let mut out_buffer = Vec::new(); // TODO: perhaps specify the capacity?
		timed!("best_plays > encoding", {
			...
			println!("Encoding {}x{} image", image.width(), image.height());
			let mut cursor = Cursor::new(&mut out_buffer);
			image.write_to(&mut cursor, image::ImageFormat::WebP)?;
		});

		timed!("best_plays > reply", {
			println!("Sending {} bytes", out_buffer.len());
